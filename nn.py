...
class Graph(object):
    ...
    def get_inputs(self, node):
        ...
        """
        "*** YOUR CODE HERE ***"
        return [self.get_output(i) for i in node.get_parents()]
    ...
    def step(self, step_size):
        ...
        for n in self.inputs:
            n.data = n.data - (self.get_gradient(n) * step_size)
...
class MatrixMultiply(FunctionNode):
    ...
    def forward(inputs):
        "*** YOUR CODE HERE ***"
        #assert inputs[0].shape(1) == inputs[1].shape(0), "inner input dimensions must match"
...
class MatrixVectorAdd(FunctionNode):
    ...
    def forward(inputs):
        "*** YOUR CODE HERE ***"
        #assert inputs[0].shape(1) == len(inputs[1]), "input dimensions must match"
...
class SquareLoss(FunctionNode):
    ...
    def forward(inputs):
        ...
        a = inputs[0]
        b = inputs[1]
        #assert a.shape(1) == b.shape(1), "inputs must have same shape"
        return np.mean(0.5 * np.square(a - b))
    ...
    def backward(inputs, gradient):
        ...
        b = inputs[1]
        s = a.size
        return [(gradient*(a - b))/s, - ((a - b)*gradient)/s]
